function P = TH_FDM(X, Y, U, V, U_MEAN, V_MEAN, rho, nu)
...
node = nodeID(U_MEAN);
[dP_dx, dP_dy, f] = TH(X,Y,U_MEAN,V_MEAN,U,V,rho,nu,node);
...
[a2,a1] = size(U_MEAN);
if dlet_BC == 1
    dlet_ind = {sub2ind(size(f),2*ones(1,a1-4),3:a1-2),sub2ind(size(f),(a2-1)*ones(1,a1-4),3:a1-2)};	% nodes
    dlet_d = {'north','south'};                                 % direction
    dlet_value = {0.5*rho*( mean(U_MEAN(2,3:a1-2))^2  - (U(2,3:a1-2).^2 + V(2,3:a1-2).^2) ), ...
        0.5*rho*( mean(U_MEAN(end-1,3:a1-2))^2  - (U(end-1,3:a1-2).^2 + V(end-1,3:a1-2).^2) )};
