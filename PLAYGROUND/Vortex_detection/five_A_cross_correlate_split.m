...
for d = 1:numBaseDirs
    ...
    raw_signals = { ...
        Data_below{1}.V_svd(:,1),...
        Data_below{1}.V_svd(:,2),...
    ...
    for seg = 1:nSegs
        ...
        signals = cell2mat(signals);
        valid_signal_names = signal_names(valid_indices);
        num_signals =length(valid_signal_names);
    ...
    % save figure 
    saveas(gcf, fullfile(statistics_dir, 'Averaged_split_Cross_Correlation_Functions.fig'));
    saveas(gcf, fullfile(statistics_dir, 'Averaged_split_Cross_Correlation_Functions.png'));
    ...
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    % save figure
    saveas(gcf, fullfile(statistics_dir, 'Peak_split_Cross_Correlation.fig'));
    saveas(gcf, fullfile(statistics_dir, 'Peak_split_Cross_Correlation.png'));
    ...
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    % save figure
    saveas(gcf, fullfile(statistics_dir, 'Time_split_Shifts.fig'));
    saveas(gcf, fullfile(statistics_dir, 'Time_split_Shifts.png'));
