...
for i = 1:length(base)
    ...
    for batch = 1:numBatches
        dataloc_below = fullfile(base{i}, 'Statistics', num2str(imageCount), ...
    ...
    k_below = find(cumulativeEnergy_below > real_signal, 1, 'first');
    disp(['The number of modes required to capture ', num2str(real_signal), '% of the energy is ', num2str(k_below)]);
    POD_rebuild = struct();
    POD_rebuild.modes_required = k_below;
    POD_rebuild.SNR = SNR;
    ...
    for imNo = 1: imageCount
        batchNum = ceil(imNo / 3600);
        kdx = imNo - (batchNum - 1) * caseImages;
        %% recreate flow 
        dataloc = (fullfile(base{i}, 'CalibratedPIV', num2str(imageCount),['Cam', num2str(CameraNo)], 'Instantaneous',endpoint));
        % Load Co_ords.mat
        ...
        UV_rec_below = Data_below{batchNum}.U_svd(:, 1:k_below) * ...
            (Data_below{batchNum}.S_svd(1:k_below, 1:k_below) * Data_below{batchNum}.V_svd(kdx, 1:k_below)');
        ...
        u(first_row_index+1:end,:) = u_rec_below(first_row_index+1:end,:);
        v(first_row_index+1:end,:) = v_rec_below(first_row_index+1:end,:);
...
% 
            saveas(figure(1), fullfile(base{i}, 'Statistics', num2str(imageCount), ['Cam', num2str(CameraNo)], 'Instantaneous', 'Calibrated', sprintf('vortex_break_down_split_%05d.fig', imNo)));
            saveas(figure(1), fullfile(base{i}, 'Statistics', num2str(imageCount), ['Cam', num2str(CameraNo)], 'Instantaneous', 'Calibrated', sprintf('vortex_break_down_split_%05d.jpg', imNo)));
%             close(figure(1));
            close(gcf)
        end
    ...
    save_path = fullfile(save_dir, 'cavity_stats_split.mat');
    


    % Process X & Y Locations
    for k = 1:size(vortexCentersY_all, 2)
        frequencies.vortexCenterY(k) = process_and_plot(vortexCentersY_all(:, k), ['vortexCenterY_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
        frequencies.vortexCenterX(k) = process_and_plot(vortexCentersX_all(:, k), ['vortexCenterX_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
        frequencies.rotation(k) = process_and_plot(rotation_all(:,k), ['rotationRate_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
        frequencies.vortexSize(k) = process_and_plot(vortexSize_all(:,k), ['VortexSize_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
    
    end
    
    for k = 1:size(x_loc,2)
        frequencies.deltaStar(k) = process_and_plot(deltaStar_all(:,k), ['delta_star_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
        frequencies.theta(k) = process_and_plot(momentumThickness_all(:,k), ['Momentum thickness_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
    end
    
    
    for k = 1:size(v_flux_all,2)
        frequencies.vflux(k) = process_and_plot(v_flux_all(:,k), ['vflux_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
        frequencies.convective_velocity(k) = process_and_plot(convective_velocity(:,k), ['convective velocity_split_' num2str(k)], save_dir, samplingRate(i), segment_length, overlap, nfft);
