...
%     data = load(fullfile(statistics_dir, 'cavity_stats_split.mat'));
    numBatches = N / segmentLength;
    Data_above = cell(1, numBatches);
    Data_below = cell(1, numBatches);
    
    for batch = 1:numBatches
        dataloc_below = fullfile(base_dir, 'Statistics', num2str(N), ...
                                    ['Cam' num2str(CameraNo)], 'Instantaneous', ...
                                    'Calibrated','Below', num2str(batch));
        dataloc_above = fullfile(base_dir, 'Statistics', num2str(N), ...
                                    ['Cam' num2str(CameraNo)], 'Instantaneous', ...
    ...
    for seg = 1:nSegs
        ...
        signals = { ...
            Data_below{seg}.V_svd(:,1),...
            Data_below{seg}.V_svd(:,2),...
            Data_below{seg}.V_svd(:,3),...
            Data_above{seg}.V_svd(:,1),...
            Data_above{seg}.V_svd(:,2),...
            Data_above{seg}.V_svd(:,3),...
            Data_above{seg}.V_svd(:,4),...
            Data_above{seg}.V_svd(:,5),...
            Data_above{seg}.V_svd(:,6),...
            Data_above{seg}.V_svd(:,7),...
            Data_above{seg}.V_svd(:,8),...
            Data_above{seg}.V_svd(:,9),...
            Data_above{seg}.V_svd(:,10),...
        };
    ...
    % save figure 
    saveas(gcf, fullfile(statistics_dir, 'Averaged_MODAL_split_Cross_Correlation_Functions.fig'));
    saveas(gcf, fullfile(statistics_dir, 'Averaged_MODAL_split_Cross_Correlation_Functions.png'));
    ...
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    % save figure
    saveas(gcf, fullfile(statistics_dir, 'Peak_Cross_MODAL_split_Correlation.fig'));
    saveas(gcf, fullfile(statistics_dir, 'Peak_Cross_MODAL_split_Correlation.png'));
    ...
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    % save figure
    saveas(gcf, fullfile(statistics_dir, 'Time_MODAL_split_Shifts.fig'));
    saveas(gcf, fullfile(statistics_dir, 'Time_MODAL_split_Shifts.png'));
