...
samplingRate =[250,100,250,100,250,100,250,100,250,]; %hz 
...
%         tempgamma(b_mask) =0;
        ...
        row_mean_abs_u = mean(gammaFieldLocation, 2, 'omitnan');
        
        % Create a logical mask for rows where the average is above 0.01
        valid_rows = (abs(row_mean_abs_u) > 0.2);
        first_nan_row = find(all(isnan(gammaFieldLocation), 2), 1, 'first');

        % If a NaN row is found, invalidate 10 rows above it
        if ~isempty(first_nan_row) && first_nan_row > 10
            valid_rows(first_nan_row - 10 : end) = false;
        end
        
        % Apply a moving maximum filter to pad out the ones by 3 on each side
        padded_valid_rows = movmax(valid_rows, [5 5]);
        
        % Expand to 2D to match gammaField dimensions
        valid_mask_2D = repmat(padded_valid_rows, 1, size(v, 2));
