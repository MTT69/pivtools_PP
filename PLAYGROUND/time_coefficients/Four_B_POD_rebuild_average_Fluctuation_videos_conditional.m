...
scaleFactor = 9.53;
        ...
        for run = 1:length(base)
            ...
            cavity_stats = load(fullfile(statsloc, "cavity_stats.mat"));

            load(fullfile(dataloc_below, 'autocorr_results.mat'));
            ...
            for k = modes
                ...
                for p_t_loop = 1:2
                    ...
                    if strcmp(p_t, 'peak')
                        peak_data = load(fullfile(dataloc_below, ['peak_indices_Mode', num2str(k), '_', num2str(w), 'x', num2str(h), '.mat']));
                    ...
                    elseif strcmp(p_t, 'trough')
                        trough_data = load(fullfile(dataloc_below, ['trough_indices_Mode', num2str(k), '_', num2str(w), 'x', num2str(h), '.mat']));
                    ...
                    ycorners_top = [Co_ords.Co_ords(i).y(1, 1), Co_ords.Co_ords(i).y(index, end)];
                    xcorners_top = [Co_ords.Co_ords(i).x(1, 1), Co_ords.Co_ords(i).x(index, end)];
                    ...
                    for H = 1:numCells
                        developmentVideo_top{H} = zeros(size(VelData.piv_result(i).ux(1:index,:))); % Match Total_flow_top dimensions
                        developmentVideo_bottom{H} = zeros(size(VelData.piv_result(i).ux(index+1:end,:))); % Match Total_flow_bottom dimensions
                        developmentVideo_ux_top{H} = zeros(size(VelData.piv_result(i).ux(1:index,:))); % Match Total_flow_top dimensions
                        developmentVideo_uy_top{H} = zeros(size(VelData.piv_result(i).ux(1:index,:))); % Match Total_flow_top dimensions
                        developmentVideo_ux_bottom{H} = zeros(size(VelData.piv_result(i).ux(index+1:end,:))); % Match Total_flow_bottom dimensions
                        developmentVideo_uy_bottom{H} =zeros(size(VelData.piv_result(i).ux(index+1:end,:))); % Match Total_flow_bottom dimensions
                    ...
                    for imNo = combined_indices
                        ...
                        u_rec_above = UV_rec_above(1:h*w)+ Data_above.mean_u;
                        v_rec_above = UV_rec_above(h*w+1:end) + Data_above.mean_v;
                        ...
                        u_rec_below = UV_rec_below(1:h*w)+ Data_below.mean_u;
                        v_rec_below = UV_rec_below(h*w+1:end) + Data_below.mean_v;
                        ...
                        % total fluctating
                        fluc_top = fluc(1:index,:);
                        fluc_bottom = fluc(index+1:end,:);
                        ux_top = ux(1:index,:);
                        uy_top = uy(1:index,:);
                        ux_bottom = ux(index+1:end,:);
                        uy_bottom = uy(index+1:end,:);
                    ...
                    mp4_filename = fullfile(dataloc_below, ['FlowDevelopment-average', p_t, '-', plot_type, '-Mode', num2str(k), '_', num2str(w), 'x', num2str(h), '.mp4']);
                    ...
                    stat_path = (fullfile(dataloc_below,['energydevelopment', p_t, '-', plot_type, '-Mode', num2str(k), '_', num2str(w), 'x', num2str(h), '.mat']));
                    ...
                    for H = 1:numCells
                        ...
                        plotTimeInstanceGraph( b_mask, xcorners_top, ycorners_top, developmentVideo_top{H}, lower_top, upper_top, ...
