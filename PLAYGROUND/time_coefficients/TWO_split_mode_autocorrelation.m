...
for base_idx = 1:length(base_dir)
    ...
    [~, baseString, ext] = fileparts(base_dir{base_idx});
    Camera = 1;
    window = [16 16];
    i = 5;
    lower_limit = -0.25;
    upper_limit = 0.25;
    timecoefficient = zeros(3,N);
    statistics = (fullfile(base_dir{base_idx}, 'Statistics', num2str(N), ['Cam' num2str(CameraNo)], 'Instantaneous', 'Calibrated','Below'));
    for batch = 1 : (N/caseImages)
        dataloc = (fullfile(base_dir{base_idx}, 'Statistics', num2str(N), ['Cam' num2str(CameraNo)], 'Instantaneous', 'Calibrated','Below',num2str(batch)));
    ...
    for mode = 1:3
        ...
        saveas(gcf, fullfile(statistics, ['autocorr_mode_split_', num2str(mode), '.png']));
        saveas(gcf, fullfile(statistics, ['autocorr_mode_split_', num2str(mode), '.fig']));
        close gcf
    end
    close all
    % Save the results structure
    save(fullfile(statistics, 'autocorr_results_split.mat'), 'results');
    ...
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    % Save the figure
    saveas(gcf, fullfile(statistics, 'autocorrelation_modes_split_1_3.jpg'));
